...
export default function ModerationPage() {
    ...
    void (async () => {
      ...
      if (!syncRes.ok) {
        ...
        if (isRbacError) {
          ...
          console.warn(
            '[ModerationPage] syncApproveToServer returned',
            syncRes.httpStatus,
            '(RBAC) — keeping local approval.',
          );
        } else {
  ...
  const reject = async (
    ...
    const combinedRejectNotes = [sessionRejectDraft, expertNotesPayload]
      .filter(Boolean)
      .join('\n\n');
    ...
    void (async () => {
      ...
      if (!syncRes.ok) {
        ...
        if (isRbacError) {
          ...
          console.warn(
            '[ModerationPage] syncRejectToServer returned',
            syncRes.httpStatus,
            '(RBAC) — keeping local rejection.',
          );
        } else {
                ...
                {selectedId &&
                  (() => {
                    ...
                    if (item.mediaType === 'youtube' && item.youtubeUrl?.trim()) {
                      ...
                      (item.audioData?.trim() || item.audioUrl?.trim())
                    ) {
                    ...
                    const convertedForPlayer =
                      item && (selectedItemFull ?? item)
                        ? (() => {
                            ...
                            return {
                              ...
                              waveformUrl: '',
                              coverImage: '',
                              instruments: (r.culturalContext?.instruments || []).map(
                                (name, idx) => ({
                                  id: `li-${idx}`,
                                  name,
                                  nameVietnamese: name,
                    ...
                    const uploadedAtLabel = formatDateTime(
                      (item as LocalRecordingMini & { uploadedDate?: string }).uploadedDate ||
                        item.uploadedAt,
                    );
                    ...
                    const missingInfoCount = infoRows.filter((row) =>
                      isPlaceholderField(row.value),
                    ).length;
                      ...
                      <div className="space-y-6">
                        ...
                        <ModerationSubmissionDetailPanels
                          ...
                          expertReviewNotesDraft={
                            item.id ? (expertReviewNotesDraft[item.id] ?? '') : ''
                          }
          ...
          {activeTab === 'ai' && (
            ...
            >
              ...
              <p className="text-neutral-600 text-sm mb-6 max-w-2xl">
                Xem lại các phản hồi do AI tạo ra (ví dụ từ Hỏi đáp thông minh). Nếu phát hiện sai
                sót, hãy báo lỗi và ghi chú để điều chỉnh.
              </p>
              {!aiResponsesLoaded ? (
                <div className="text-neutral-500 text-sm">Đang tải...</div>
              ) : aiResponses.length === 0 ? (
                <div
                  className="rounded-2xl border-2 border-dashed border-neutral-200/80 p-8 text-center"
                ...
                >
                  ...
                  <p className="text-neutral-500 text-sm mt-1">
                    Khi có câu trả lời AI từ Cổng nghiên cứu (Hỏi đáp thông minh) hoặc chatbot,
                    chúng sẽ xuất hiện tại đây để chuyên gia xem và báo lỗi nếu cần.
                  </p>
                </div>
              ) : (
                <ul className="space-y-4">
                  {aiResponses.map((item) => (
                    ...
                    >
                      ...
                      <p className="text-neutral-700 text-sm mb-4 whitespace-pre-wrap">
                        {item.answer}
                      </p>
                      {item.citations && item.citations.length > 0 && (
                        <div className="mb-4 rounded-xl border border-sky-200/80 bg-sky-50/60 p-3">
                          <p className="text-xs font-semibold text-sky-800 mb-1">
                            Nguồn trích dẫn do Researcher gửi kèm
                          </p>
                            ...
                            {item.citations.map((citation, idx) => (
                              <li key={`${item.id}-citation-${idx}`}>
                                <button
                                  type="button"
                                  onClick={() =>
                                    navigate(`/recordings/${citation.recordingId}`, {
          ...
          {activeTab === 'knowledge' && (
            ...
            >
              ...
              <p className="text-neutral-600 max-w-md">
                Bách khoa toàn thư cộng tác về âm nhạc truyền thống Việt Nam.
              </p>
            </div>
          )}
        </div>
                  ...
                  <div className="rounded-2xl shadow-md border border-secondary-200/70 bg-gradient-to-b from-[#FFFCF5] to-secondary-50/50 p-8">
                    <div className="flex flex-col items-center gap-4 mb-2">
                      ...
                      <div className="text-neutral-700 text-center space-y-1">
                        <p>
                          Bản thu sẽ được trả lại danh sách để các chuyên gia khác có thể nhận kiểm
                          duyệt.
                        </p>
                ...
                <div className="overflow-y-auto p-6">
                  <div className="rounded-2xl shadow-md border border-secondary-200/70 bg-gradient-to-b from-[#FFFCF5] to-secondary-50/50 p-8">
                    ...
                    <div className="mt-4 text-left max-w-xl mx-auto w-full">
                      ...
                      />
                      <p
                        id="moderation-approve-notes-hint"
                        className="mt-2 text-xs text-neutral-700"
                      >
                        Ghi chú được lưu cùng trạng thái kiểm duyệt (Phase 1 Spike) và hiển thị
                        trong chi tiết bản thu.
                      </p>
                      <p
                        id="moderation-approve-shortcuts"
                  ...
                  <div className="rounded-2xl shadow-md border border-secondary-200/70 bg-gradient-to-b from-[#FFFCF5] to-secondary-50/50 p-8">
                    <div className="flex flex-col items-center gap-4 mb-4">
                      ...
                      >
                        ...
                        {rejectType === 'direct' ? 'từ chối vĩnh viễn' : 'từ chối tạm thời'} bản thu
                        này?
                      </h3>
                      <div className="text-neutral-800 text-center text-sm space-y-1 max-w-lg">
                        <p>
                    ...
                    <div className="mt-4 text-left max-w-xl mx-auto w-full">
                      ...
                      >
                        Ghi chú chuyên gia{' '}
                        <span className="text-red-800 font-semibold">(nên điền)</span>
                      </label>
                      <textarea
                      ...
                      />
                      <p
                        id="moderation-reject-confirm-notes-hint"
                        className="mt-2 text-xs text-neutral-800"
                      >
                        Nội dung được lưu cục bộ (Phase 1 Spike). Đã sao chép từ lý do từ chối — bạn
                        có thể chỉnh sửa.
                      </p>
                      <p
                        id="moderation-reject-confirm-shortcuts"
                        className="mt-1 text-xs text-neutral-600"
                      >
          ...
          createPortal(
            ...
            >
              ...
              >
                ...
                <div className="flex items-center justify-between p-6 border-b border-secondary-200/80 bg-gradient-to-b from-[#FFF8EA] to-cream-50/80">
                  <h2
                    id="moderation-reject-warning-title"
                    className="text-2xl font-bold text-neutral-900"
                  >
                    Cảnh báo
                  </h2>
                ...
                <div className="overflow-y-auto p-6">
                  <div className="rounded-2xl shadow-md border border-secondary-200/70 bg-gradient-to-b from-[#FFFCF5] to-secondary-50/50 p-8">
                    <div className="flex flex-col items-center gap-4 mb-2">
                      ...
                      <div className="text-neutral-700 text-center space-y-1">
                        <p>
                          Cho dù từ chối tạm thời hay từ chối vĩnh viễn, bạn đều cần phải nhập lý do
                          từ chối.
                        </p>
                      </div>
                    </div>
              ...
              >
                ...
                >
                  <div className="flex items-center justify-between p-6 border-b border-neutral-200/80 bg-gradient-to-br from-neutral-700 to-neutral-800">
                    <h2
                      id="moderation-delete-dialog-title"
