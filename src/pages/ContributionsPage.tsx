...
export default function ContributionsPage() {
    ...
    const editingObj = {
      ...
      audioData:
        effectiveMediaType === 'audio'
          ? rec?.audioFileUrl || (rec as SubmissionRecordingMedia).audioUrl
          : null,
  ...
  const renderSubmissionCard = (sub: Submission) => {
    ...
    const instrumentLabel = sub.recording?.instrumentIds?.length
          ...
          <div className="flex flex-wrap items-center justify-between gap-y-3 mt-4 text-sm text-neutral-600">
            <div className="flex flex-wrap items-center gap-x-5 gap-y-2">
              ...
              {(sub.recording?.performanceContext ||
                (sub.recording?.instrumentIds && sub.recording.instrumentIds.length > 0)) && (
                <div
      ...
      {!loading && !error && submissions.length === 0 && (
        <div className="py-10 text-center">
          ...
          <p className="mx-auto max-w-md font-medium leading-relaxed text-neutral-600">
            ...
            {activeStatusTab === 'ALL' &&
              'Bạn chưa có đóng góp nào. Hãy thử tải lên từ trang Đóng góp.'}
          </p>
        </div>
      )}

      {!loading && !error && submissions.length > 0 && (
        <div className="space-y-4">{submissions.map((sub) => renderSubmissionCard(sub))}</div>
      )}
            ...
            <div className="lg:grid lg:grid-cols-[minmax(260px,320px)_minmax(0,1fr)] lg:gap-8 xl:gap-10 lg:items-start">
              ...
              >
                ...
                <p className="mt-2 text-sm font-medium text-neutral-700">
                  Các bước sàng lọc do đội ngũ duyệt thực hiện; dùng tab bên dưới để lọc đóng góp
                  theo trạng thái.
                </p>
              ...
              <main className="min-w-0">
                <details className="group mb-6 rounded-2xl border border-secondary-200/50 bg-gradient-to-br from-[#FFFCF5] via-cream-50/85 to-secondary-50/45 p-4 shadow-md backdrop-blur-sm lg:hidden open:shadow-lg">
                  ...
                  <div className="mt-4 border-t border-secondary-200/50 pt-4">
                    {renderModerationLegendPills(true)}
                  </div>
                </details>
              ...
              {detailSubmission && (
                <div className="space-y-4">
                  ...
                  <div className="space-y-1 rounded-xl border border-secondary-200/50 bg-gradient-to-br from-[#FFFCF5] to-cream-50/60 p-4">
                    ...
                    {renderDetailField(
                      'Bối cảnh biểu diễn',
                      formatPerformanceType(detailSubmission.recording?.performanceContext),
                    )}
                    {detailSubmission.recording?.instrumentIds &&
                      detailSubmission.recording.instrumentIds.length > 0 && (
                    ...
                    {renderDetailField(
                      'Thời lượng',
                      formatDuration(detailSubmission.recording?.durationSeconds),
                    )}
                    {renderDetailField(
                      'Kích thước',
                      formatSize(detailSubmission.recording?.fileSizeBytes),
                    )}
                    {renderDetailField(
                      'Ngày ghi âm',
                      formatDate(detailSubmission.recording?.recordingDate || null),
                    )}
                    {renderDetailField('Lời gốc', detailSubmission.recording?.lyricsOriginal)}
                    {renderDetailField(
                      'Lời tiếng Việt',
                      detailSubmission.recording?.lyricsVietnamese,
                    )}
                    ...
                    {detailSubmission.recording?.gpsLatitude != null &&
                      detailSubmission.recording?.gpsLongitude != null &&
                      (detailSubmission.recording.gpsLatitude !== 0 ||
                        detailSubmission.recording.gpsLongitude !== 0) &&
                      renderDetailField(
                        'Tọa độ GPS',
                        `${detailSubmission.recording.gpsLatitude}, ${detailSubmission.recording.gpsLongitude}`,
                      )}
                  </div>
                  ...
                  {detailSubmission.recording && (
                    <div className="mb-6">
                      {(() => {
                        ...
                        if (audioSrc) {
                          return <AudioPlayer src={audioSrc} title={title} artist={performer} />;
                        }

                        // Fallback: if videoURL exists but doesn't match video patterns, it might be an audio file in video field
                        if (videoSrc) {
                          return <AudioPlayer src={videoSrc} title={title} artist={performer} />;
                        }
                ...
                <button
                  ...
                  className={cn(
                    ...
                    isRequestingEdit
                      ...
                      : 'cursor-pointer hover:from-primary-500 hover:to-primary-600 hover:shadow-lg hover:scale-[1.02] active:scale-95',
                  )}
                  onClick={async () => {
                    setIsRequestingEdit(true);
                    try {
                      const res = await submissionService.requestEditSubmission(
                        detailSubmission.id,
                      );
                      if (res?.isSuccess) {
                        uiToast.success(
                          notifyLine(
                            'Thành công',
                            'Yêu cầu chỉnh sửa đã được gửi đến ban quản trị.',
                          ),
                        );
                        setDetailSubmission(null);
                        loadSubmissions();
                        ...
                        uiToast.error(
                          notifyLine('Lỗi', res?.message || 'Không thể gửi yêu cầu chỉnh sửa.'),
                        );
                      }
                    } catch (err: unknown) {
                      ...
                      uiToast.error(
                        notifyLine(
                          'Lỗi',
                          apiMsg || fallback || 'Đã xảy ra lỗi khi gửi yêu cầu chỉnh sửa.',
                        ),
                      );
                    } finally {
