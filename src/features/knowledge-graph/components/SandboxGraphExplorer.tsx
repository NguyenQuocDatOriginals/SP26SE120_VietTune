...
import { getGroupLabel, getLinkLabel, getRelationshipLabel } from '@/features/knowledge-graph/utils/sandboxGraphLabels';
...
export interface SandboxGraphExplorerProps {
  embedded?: boolean;
}

export default function SandboxGraphExplorer({ embedded = false }: SandboxGraphExplorerProps) {
  ...
  const rootClass = embedded
    ? 'w-full h-[calc(100vh-240px)] min-h-[520px] bg-slate-50 flex flex-col rounded-xl border border-slate-200 overflow-hidden'
    : 'w-full h-screen bg-slate-50 flex flex-col';

  return (
    <div className={rootClass}>
      {/* Top Bar */}
      <div
        className={`bg-white border-b border-slate-200 shrink-0 flex items-center justify-between gap-4 ${
          embedded ? 'px-3 py-2' : 'p-4'
        }`}
      >
        <div className="flex items-center gap-2.5">
          ...
          >
            {embedded ? 'Biểu đồ tri thức' : 'Bản đồ Tri thức Âm nhạc (VietTune)'}
              ...
              {graphData.nodes.map(node => {
                ...
                return (
                  <g
                    ...
                    onDoubleClick={() => void handleNodeDoubleClick(node)}
                  >
                    <circle
                    ...
                    <button
                      ...
                      onClick={() => {
                        if (expandedNodes.has(selection.id)) {
                          ...
                          void handleNodeDoubleClick(selection);
                        }
                      }}
                    >
              ...
              <div className="flex flex-col gap-2">
                {Array.from(new Set(expandPopupData.detail.neighbors.map(n => n.relationType))).map(relType => {
                  ...
                  return (
                    <label
                      key={relType}
                      aria-label={`${labelText} (${count} thực thể liên quan)`}
