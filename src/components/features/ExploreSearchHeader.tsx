...
export default function ExploreSearchHeader({
  ...
  const tabBtn = (
    ...
    <button
      ...
      aria-controls={
        m === 'keyword' ? 'explore-search-panel-keyword' : 'explore-search-panel-semantic'
      }
      tabIndex={mode === m ? 0 : -1}
  ...
  return (
    ...
    >
      ...
      {!homeSemanticOnly ? (
        ...
        >
          {tabBtn(
            ...
            'Tìm theo từ khóa',
            <Search
              className="h-4 w-4 shrink-0 text-secondary-700"
              strokeWidth={2.25}
              aria-hidden
            />,
            tabKeywordRef,
      ...
      {!homeSemanticOnly ? (
        ...
        >
          <p className="mb-3 text-sm font-medium text-neutral-600">
            Gõ từ khóa rồi nhấn Enter hoặc <span className="text-neutral-800">Tìm</span> — kết hợp
            với bộ lọc bên trái khi cần.
          </p>
      ...
      >
        <p className="mb-3 text-sm font-medium text-neutral-600">
          Mô tả bằng câu tự nhiên; hệ thống xếp hạng bản thu theo độ khớp nghĩa trên tập đang tải
          (có thể kết hợp bộ lọc).
        </p>
        <div className="relative w-full min-h-[52px] rounded-xl border border-secondary-200/80 bg-gradient-to-r from-secondary-50/80 to-cream-50/90 px-3 py-2 shadow-sm transition-all focus-within:border-secondary-400 focus-within:ring-2 focus-within:ring-secondary-300/40 sm:min-h-[56px] sm:px-4 sm:py-2.5">
          <Sparkles
