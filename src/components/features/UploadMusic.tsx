...
import {
  PROVINCE_REGION_CODE_TO_NAME as REGION_CODE_TO_NAME,
  ...
  return (
    <div ref={dropdownRef} className="relative">
      <button
        ...
        disabled={disabled}
        className={`w-full px-5 py-3 pr-10 text-neutral-900 border border-neutral-400 rounded-xl focus:outline-none focus:border-primary-500 transition-all duration-200 text-left flex items-center justify-between shadow-sm hover:shadow-md ${
          disabled ? 'opacity-50 cursor-not-allowed' : 'cursor-pointer'
        }`}
        style={{ backgroundColor: '#FFFCF5' }}
      >
        <span
          className={value ? 'text-neutral-900 font-medium text-sm' : 'text-neutral-400 text-sm'}
        >
          {value || placeholder}
        </span>
        <ChevronDown
                      ...
                      {lunarMonths.map((lm) => {
                        const isFutureLunarMonth =
                          ...
                          (lm.month > currentLunar.month ||
                            (lm.month === currentLunar.month && lm.leap && !currentLunar.leap));
                        return (
                          <button
...
function TextInput({
  ...
  const baseClasses = `w-full px-5 py-3 text-neutral-900 placeholder-neutral-500 border border-neutral-400 focus:outline-none focus:border-primary-500 transition-colors ${
...
export default function UploadMusic({ recordingId, isApprovedEdit }: UploadMusicProps = {}) {
  ...
  const handleConfirmSubmit = async (isFinal: boolean = true) => {
    ...
    try {
      ...
      const districtId = districtsData.find(
        (d) => d.name === district && d.provinceId === provinceId,
      )?.id;
      const selectedCommuneId = communesData.find(
        (c) => c.name === commune && c.districtId === districtId,
      )?.id;
      ...
      const payload: RecordingDto = {
        ...
        recordingDate: recordingDate
          ? new Date(recordingDate).toISOString()
    ...
    <React.Fragment>
      <form onSubmit={handleSubmit} className="w-full space-y-6">
        ...
        <MetadataStepSection
          ...
          SearchableDropdownComponent={SearchableDropdown}
