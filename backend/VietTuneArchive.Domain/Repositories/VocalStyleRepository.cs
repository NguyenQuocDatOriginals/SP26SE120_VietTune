...
namespace VietTuneArchive.Domain.Repositories
{
    public class VocalStyleRepository : GenericRepository<VocalStyle>, IVocalStyleRepository
    {
        ...
        public async Task<IEnumerable<VocalStyle>> SearchByNameAsync(string? name)
        {
            var vocalStyles = await _context.VocalStyles.ToListAsync();
            if (string.IsNullOrWhiteSpace(name))
                return vocalStyles;
            string normalizedSearchName = RemoveVietnameseDiacritics(name).ToLower();
            return vocalStyles
                .Where(vs => RemoveVietnameseDiacritics(vs.Name ?? "").ToLower().Contains(normalizedSearchName))
                .ToList();
        }
    }
}
